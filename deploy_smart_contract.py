...
class SmartContract:
    myAddress = "0xEb36725C0EBA39acc477ac8af37fAdA7Ea3ae921"
    privateKey = "0xe0fccc41f83cf8524c72d4b2c1f6e17d06c48f3d4192d07a51d21d2c23727915"
    # get the bytecode
            ...
            self.w3.eth.send_raw_transaction(
                self.w3.eth.account.sign_transaction(
                    self.deploy_smart_contract.functions.sendMessage(friend_key=receiverAddress,
                        ...
                        "gasPrice": self.w3.eth.gas_price, "chainId": self.chainId, "from": self.myAddress,
                        "nonce": self.w3.eth.getTransactionCount(self.myAddress) + 1
                    }), self.privateKey).rawTransaction))
    ...
    def method_name(self):
        friendAddress = "0x54F08CBFF5E3E71BA0D7f4248af2c8D519B8958D"

        friendName = "laith"

        print("the nonce number" + str(self.nonce))

        print(self.create_account("laith"))

        nonce = self.w3.eth.getTransactionCount(self.myAddress)
        print("the nonce number after creating account " + str(nonce))

        msg = input(f"write some words to  {friendName}")

        print(self.addFriend(friendName=friendName, friendAddress=friendAddress))

        nonce = self.w3.eth.getTransactionCount(self.myAddress) - 1

        print("the nonce number after adding friend " + str(nonce))
        self.sendMessage(msg=msg, receiverAddress=friendAddress)

        nonce = self.w3.eth.getTransactionCount(self.myAddress)
        print("the nonce number after send message " + str(nonce))

        print("your messages is ")
        print(self.readMessages(friendAddress=friendAddress))

        print("the nonce number after reading message " + str(nonce))


c = SmartContract(HTTP="HTTP://127.0.0.1:7545",chainId=1337,publicKey= "0xEb36725C0EBA39acc477ac8af37fAdA7Ea3ae921",privateKey="0xe0fccc41f83cf8524c72d4b2c1f6e17d06c48f3d4192d07a51d21d2c23727915",name="ahmad")
print(c.nonce)
